...
export default function NotesSection({
  ...
  const communityNotes = user ? notes.filter((n) => n.authorId !== user.id) : []
  const scoped = tab === "mine" ? myNotes : communityNotes
  ...
  const headerCount = tab === "mine" ? myNotes.length : communityNotes.length
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
        <SectionHeader
          ...
          sub={
            user
              ? `${
                  tab === "mine" ? "your notes" : "community notes"
